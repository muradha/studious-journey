...
Public Class FormBarangMasuk
    ...
    Sub TotalItem()
        ...
        For I As Integer = 0 To DGV.Rows.Count - 1
            HitungItem = HitungItem + Val(DGV.Rows(I).Cells(4).Value)
    ...
    Private Sub BtnSimpan_Click(sender As Object, e As EventArgs) Handles BtnSimpan.Click
        ...
        Else
            ...
            If Not rd.HasRows Then
                Dim simpanmaster As String = "INSERT INTO barang_masuk(faktur,tanggal,kode_petugas,kode_supplier) values('" & TxtFaktur.Text & "','" & Format(DtTgl.Value, "yyyy/MM/dd") & "','" & "Admin" & "','" & TxtKode.Text & "')"
                cmd = New OracleCommand(simpanmaster, conn)
                cmd.ExecuteNonQuery()
                For baris As Integer = 0 To DGV.Rows.Count - 2
                    Dim sqlsimpan As String = "INSERT INTO barang_masuk_detail(faktur, kode_barang,nama_barag,jumlah) values('" & TxtFaktur.Text & "','" & DGV.Rows(baris).Cells(0).Value & "','" & DGV.Rows(baris).Cells(1).Value & "','" & DGV.Rows(baris).Cells(2).Value & "','" & DGV.Rows(baris).Cells(3).Value & "','" & DGV.Rows(baris).Cells(4).Value & "')"
                    ...
                    rd.Read()
                    If rd.HasRows Then
                        Dim kurangistok As String = "UPDATE barang SET stock='" & rd.Item(4) + DGV.Rows(baris).Cells(4).Value & "' WHERE kode_barang='" & DGV.Rows(baris).Cells(0).Value & "'"
                        cmd = New OracleCommand(kurangistok, conn)
                        cmd.ExecuteNonQuery()
                    End If
                Next baris
